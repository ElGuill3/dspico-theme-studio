...
export class PortableProjectStore {
  ...
  async readAsset(sha256: string): Promise<Uint8Array> { if (!/^[a-f0-9]{64}$/.test(sha256)) throw new BundlePathError(sha256); const bytes = new Uint8Array(await readFile(await this.resolve(expectedPath(sha256)))); if (digest(bytes) !== sha256) throw new BundleCommitError(`Asset source hash mismatch: ${sha256}`); return bytes; }
